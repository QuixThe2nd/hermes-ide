...
A caller whose config sets `deep_research.worker_file_tools: false` runs no-file jobs instead: the flag is frozen onto `request.json` at `start` (the runner never re-consults live config mid-job), lanes are pinned to `-t web,browser` (retrieval without the filesystem), and the writer is pinned to `-t research_writer`, a deliberately empty toolset this plugin registers itself (sealed, via the plugin context's `register_toolset`) at load — it exists only while the plugin is enabled and overlays cannot widen it. Lane reports are already injected into the writer's prompt, so it needs no tools at all.
