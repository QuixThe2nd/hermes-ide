...
def _read_discord_token() -> str:
    """Resolve DISCORD_BOT_TOKEN via secret scope, then ``.env`` file fallback."""
    try:
        from agent.secret_scope import UnscopedSecretError, get_secret

        scoped = (get_secret("DISCORD_BOT_TOKEN", "") or "").strip()
        if scoped:
            return scoped
...
def _distinct_lounge_channel_name(base_name: str, user_id: str) -> str:
    """Append a short user-id suffix when the base lounge name is already taken."""
    suffix = re.sub(r"\D", "", str(user_id))[-4:] or str(user_id)[-4:]
    candidate = f"{base_name}-{suffix}"
    if len(candidate) <= _MAX_CHANNEL_NAME_LEN:
        return candidate
    trimmed_base = base_name[:_MAX_CHANNEL_NAME_LEN - len(suffix) - 1].rstrip("-")
    return f"{trimmed_base}-{suffix}"


def _channel_in_category(
    ...
    for channel in channels:
        if str(channel.get("id") or "") != channel_id:
            continue
        return str(channel.get("parent_id") or "") == chat_category_id
    return False


def _channel_owner_user_id(state: Dict[str, Any], channel_id: str) -> str:
    for guest in state.get("guests", []):
        if isinstance(guest, dict) and guest.get("channel_id") == channel_id:
...
def _resolve_lounge_channel(
    ...
    """Return (channel_id, channel_name) for the guest's lounge under Chat."""
    tracked = next(
        (
            guest
            for guest in state.get("guests", [])
            if isinstance(guest, dict) and guest.get("user_id") == guest_user_id
        ),
        None,
    )
    if tracked:
        tracked_id = str(tracked.get("channel_id") or "")
        if tracked_id and _channel_in_category(channels, tracked_id, chat_category_id):
            return tracked_id, channel_name

    name_match_id = _find_lounge_by_name(channels, chat_category_id, channel_name)
    if name_match_id:
        owner = _channel_owner_user_id(state, name_match_id)
        if not owner or owner == guest_user_id:
            return name_match_id, channel_name
        distinct_name = _distinct_lounge_channel_name(channel_name, guest_user_id)
        distinct_id = _find_lounge_by_name(channels, chat_category_id, distinct_name)
        if distinct_id:
            distinct_owner = _channel_owner_user_id(state, distinct_id)
            if not distinct_owner or distinct_owner == guest_user_id:
                return distinct_id, distinct_name
...
def _find_chat_category_by_name(channels: List[Dict[str, Any]]) -> str:
    """The category named Chat (case-insensitive), or empty when absent."""
    for channel in channels:
...
def _resolve_chat_category(
    ...
    *,
    ...
    """Pick the Chat category for ``resolved_guild_id`` against ``channels``.

    Explicit ``chat_category_id`` args win. Saved ids are only reused when the
    guild matches and the id is still a category in ``channels``.
    """
    ...
    use_saved = (
        ...
        and saved_category_id
    )
    if use_saved:
        for channel in channels:
            if (
                str(channel.get("id") or "") == saved_category_id
...
def _find_chat_category(
    ...
    *,
    ...
    """Legacy wrapper — explicit id wins; else name lookup."""
    if chat_category_id:
...
def _parse_permission_bits(value: Any) -> int:
    try:
...
def _existing_overwrite_bits(
    ...
    """Read allow/deny from a channel's ``permission_overwrites``, else (0, 0)."""
    for channel in channels:
        if str(channel.get("id") or "") != channel_id:
            continue
        for overwrite in channel.get("permission_overwrites") or []:
            ...
            if str(overwrite.get("id") or "") != target_id:
                continue
            if overwrite.get("type") != overwrite_type:
                continue
            return (
                _parse_permission_bits(overwrite.get("allow")),
                _parse_permission_bits(overwrite.get("deny")),
            )
    return 0, 0
...
def _put_everyone_deny_view(
    ...
    existing_allow, existing_deny = _existing_overwrite_bits(
        channels, channel_id, guild_id, _OVERWRITE_TYPE_ROLE
    )
...
def _put_member_allow(
    ...
    existing_allow, existing_deny = _existing_overwrite_bits(
        channels, channel_id, user_id, _OVERWRITE_TYPE_MEMBER
    )
    ...
    if run_lockdown:
        for target_id in _lockdown_targets(lockdown_channels):
            _put_everyone_deny_view(
                token, target_id, resolved_guild_id, lockdown_channels
            )
...
def _handle_add(args: Dict[str, Any], token: str) -> str:
    ...
    old_guest = next(
        (g for g in state["guests"] if g["user_id"] == guest_user_id),
        None,
    )
    old_channel_id = str(old_guest.get("channel_id") or "") if old_guest else ""

    channel_id, channel_name = _resolve_lounge_channel(
        state,
        channels,
        chat_category_id,
        guest_user_id,
