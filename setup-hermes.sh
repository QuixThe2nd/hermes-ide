...
# Fork delta (Termux/Android): the pm bootstrap stages a pinned glibc uv
# binary, which cannot run under Termux's bionic libc. Fail loudly with a
# pointer at the fork's customized installer (scripts/install.sh keeps the
# Android path) instead of a confusing exec-format error mid-bootstrap.
if [ -n "${TERMUX_VERSION:-}" ] || [[ "${PREFIX:-}" == *"com.termux/files/usr"* ]]; then
    echo -e "${YELLOW}⚠${NC} Termux detected — setup-hermes.sh's pm-pinned uv bootstrap does not support Android/bionic."
    echo "    Use the fork's customized installer instead: scripts/install.sh"
    exit 1
fi
