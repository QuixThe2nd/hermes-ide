"""Hermeticity guards for the fallback_quota_reorder plugin test suite."""
...
def _block_real_systemd_subprocess(monkeypatch):
    """Fail fast if any fallback_quota_reorder test touches real systemctl."""
    ...
    def guarded_run(args, *run_args, **run_kwargs):
        ...
        if "systemctl" in joined:
            raise AssertionError(
                f"fallback_quota_reorder tests must not invoke real systemctl: {joined}"
            )
        for path in REAL_SYSTEMD_PATHS:
            if path in joined:
                raise AssertionError(
                    f"fallback_quota_reorder tests must not touch live systemd paths: {joined}"
                )
