...
class TestSensitivePathCheck:
    ...
    def test_hermes_config_blocked_for_write_file(self, tmp_path, monkeypatch):
        ...
        # Patch both guard homes: upstream's seam (write_guards) and the fork's
        # local override in file_tools, so the test holds under either layout.
        monkeypatch.setattr("tools.file_tools._hermes_config_resolved", str(fake_config), raising=False)
        monkeypatch.setattr("tools.file_tools._hermes_config_resolved_loaded", True, raising=False)
        monkeypatch.setattr("tools.file_tools._agent_config_writes_allowed", False, raising=False)
        monkeypatch.setattr("tools.file_tools_write_guards._agent_config_writes_allowed", False, raising=False)
    ...
    def test_hermes_config_write_allowed_with_optout(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("tools.file_tools_write_guards._hermes_config_resolved", str(fake_config))
        monkeypatch.setattr("tools.file_tools_write_guards._hermes_config_resolved_loaded", True)
        monkeypatch.setattr("tools.file_tools._agent_config_writes_allowed", True, raising=False)
        monkeypatch.setattr("tools.file_tools_write_guards._agent_config_writes_allowed", True, raising=False)
    ...
    def test_optout_reads_security_section(self, monkeypatch):
        ...
        def _permitted():
            # The opt-out lives in whichever module owns the guard post-merge.
            fn = getattr(ft, "_agent_config_writes_permitted", None) or guards._agent_config_writes_permitted
            return fn()

        def _reset_cache():
            for mod in (ft, guards):
                if hasattr(mod, "_agent_config_writes_allowed"):
                    monkeypatch.setattr(mod, "_agent_config_writes_allowed", None)
    ...
    def test_system_path_allowed(self, mock_get, monkeypatch):
        ...
        monkeypatch.setattr("tools.file_tools_write_guards._hermes_config_resolved_loaded", True)
        monkeypatch.setattr("tools.file_tools._hermes_config_resolved", "/some/other/path", raising=False)
        monkeypatch.setattr("tools.file_tools._hermes_config_resolved_loaded", True, raising=False)
        mock_ops = MagicMock()
        result_obj = MagicMock()
        result_obj.to_dict.return_value = {
