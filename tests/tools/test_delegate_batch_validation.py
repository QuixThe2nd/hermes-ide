#!/usr/bin/env python3
"""Batch input validation for delegate_agent(tasks=[...], background=False).
...
def _call(tasks):
    return json.loads(delegate_agent(tasks=tasks, parent_agent=_make_mock_parent(), background=False))
