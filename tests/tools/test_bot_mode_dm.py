...
def test_one_way_handoff_has_no_routing_parameters(monkeypatch):
    ...
    kwargs it forwards are exactly background/notify_on_complete/task_id plus
    the fixed workdir/_host_local every background delivery pins."""
    calls = _capture_spawn(monkeypatch)
    ...
    assert set(calls[0]) == {
        "command", "background", "notify_on_complete", "task_id", "workdir", "_host_local",
    }
    assert calls[0]["task_id"] == "task-7"
    assert calls[0]["_host_local"] is True
    assert Path(calls[0]["workdir"]) == Path(bot_mode_dm.__file__).resolve().parent.parent
