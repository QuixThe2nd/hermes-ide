...
class TestConfigParsing:
    ...
    def test_always_visible_dedupes_and_tolerates_invalid_entries(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda name, defer_tools=None: name in {"papercuts", "other_deferrable"},
        )
...
class TestClassification:
    def test_core_tools_never_defer(self):
        ...
        for core_name in ["terminal", "read_file", "write_file", "patch",
                          ...
                          "execute_code", "delegate_agent", "send_message"]:
...
class TestAssembly:
    ...
    def test_always_visible_tool_stays_direct_when_search_activates(self, monkeypatch):
        ...
        monkeypatch.setattr(
            tool_search,
            "is_deferrable_tool_name",
            lambda name, defer_tools=None: name in {"papercuts", "other_plugin_tool"},
        )
...
class TestBridgeDispatch:
    ...
    def test_pinned_tool_excluded_from_deferred_search_catalog(self, monkeypatch):
        ...
        monkeypatch.setattr(
            tool_search,
            "is_deferrable_tool_name",
            lambda name, defer_tools=None: name in {"pinned_policy_tool", "deferred_catalog_tool"},
        )
        config = tool_search.ToolSearchConfig.from_raw({
        ...
        result = json.loads(tool_search.dispatch_tool_search(
            {"queries": ["tool"], "limit": 10},
            current_tool_defs=defs,
            config=config,
        ))

        match_names = set(result["results"][0]["matches"])
