"""Hermeticity guards for the auto_update plugin test suite."""
...
def _block_real_systemd_subprocess(monkeypatch):
    """Fail fast if any auto_update test touches real systemctl or systemd paths."""
    ...
    def guarded_run(args, *run_args, **run_kwargs):
        ...
        if "systemctl" in joined:
            raise AssertionError(
                f"auto_update tests must not invoke real systemctl: {joined}"
            )
        for path in REAL_SYSTEMD_PATHS:
            if path in joined:
                raise AssertionError(
                    f"auto_update tests must not touch live systemd paths: {joined}"
                )
