...
class TestJobCRUD:
    ...
    def test_oneshot_in_duration_plus_repeat_greater_than_one_is_rejected(self, tmp_cron_dir):
        """'in 2m' is one-shot; --repeat 720 must not silently become a one-shot.

        Bare '2m' is now a recurring interval (upstream #e8bab87ba7). Explicit
        one-shot-by-duration is 'in 2m'.
        """
        with pytest.raises(ValueError, match="one-shot"):
            create_job(prompt="Watcher", schedule="in 2m", repeat=720)
        assert load_jobs() == []

    def test_bare_duration_keeps_explicit_repeat(self, tmp_cron_dir):
        job = create_job(prompt="Watcher", schedule="2m", repeat=720)
        assert job["schedule"]["kind"] == "interval"
