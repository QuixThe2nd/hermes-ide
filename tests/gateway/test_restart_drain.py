...
@pytest.mark.asyncio
async def test_drain_suppress_skips_notification_channel_keeps_session_ping(tmp_path, monkeypatch):
    """A suppress_notification drain marker mutes ONLY the notification-channel broadcast.
    ...
    runner, adapter = make_restart_runner()
    # A notification channel distinct from the active session's chat.
    ...
    # Exactly one send — the active-session ping to chat 999. The notification-channel
    # broadcast to restarts-42 was suppressed.
    assert len(adapter.sent_calls) == 1
    sent_chat_ids = {chat_id for chat_id, _content, _meta in adapter.sent_calls}
    assert "999" in sent_chat_ids
    assert "restarts-42" not in sent_chat_ids
