...
from gateway.config import (
    GatewayConfig,
    DeliveryTarget as ConfigDeliveryTarget,
    Platform,
    PlatformConfig,
)
...
async def test_relay_fronted_target_delivers_without_prior_inbound_chat_state(tmp_path, monkeypatch):
    """A persisted notification target must work immediately after a gateway restart."""
    ...
    config = GatewayConfig(
        platforms={
            ...
            Platform.SLACK: PlatformConfig(
                enabled=False,
                notification_channel=ConfigDeliveryTarget(
