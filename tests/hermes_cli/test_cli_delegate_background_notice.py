"""The CLI spells out auto-resume when a delegate_agent goes to the background.

A top-level ``delegate_agent`` returns a handle immediately and runs the subagent
...
def test_background_dispatch_prints_resume_notice(monkeypatch):
    ...
    cli_obj._on_tool_complete("tc1", "delegate_agent", {"goal": "x"}, result)
...
def test_background_batch_dispatch_pluralizes(monkeypatch):
    ...
    cli_obj._on_tool_complete("tc2", "delegate_agent", {"tasks": []}, result)
...
def test_synchronous_delegate_result_prints_no_notice(monkeypatch):
    ...
    cli_obj._on_tool_complete("tc3", "delegate_agent", {"goal": "x"}, result)
