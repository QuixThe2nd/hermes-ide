...
class BasePlatformAdapter(ABC):
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        _thread_metadata = _thread_metadata_for_event(event)
        # A new turn starts: stop any post-turn delegation typing supervisor
        # left running for this session before per-turn typing takes over.
        ...
        try:
            ...
            await self._flush_text_debounce_now(session_key)
            # Turn done and no queued follow-up: keep the typing bubble alive
            # while async delegations for this session run in the background.
            # (A queued follow-up fails the _should_restart check, so the drain
            # task's entry-time stop stays authoritative.)
    ...
    async def cancel_background_tasks(self) -> None:
        ...
        for _ in range(5):
            ...
            if pending:
                ...
                break
            # Loop: late-arrival tasks spawned during the wait above will be
