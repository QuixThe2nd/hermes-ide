...
_TOPLEVEL_BRIDGE: tuple = (
    ...
    *_presence(
    ...
    # Fork additions: session-reset policy section and restart-progress channel rename.
    # ``session_reset`` maps to the legacy ``default_reset_policy`` schema slot (top-level wins,
    # nested ``gateway.session_reset`` falls back); ``restart_channel_rename`` is a dict section so
