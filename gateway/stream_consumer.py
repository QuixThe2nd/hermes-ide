...
class GatewayStreamConsumer(StreamTransportMixin, StreamFallbackMixin, StreamThinkFilterMixin):
    ...
    def _clear_turn_final_flags(self) -> None:
        ...
        # (fork #48: Discord reply-ping only on the final answer).
    ...
    def _adopt_final_text(self, final_raw: str) -> None:
        ...
        # Fork #48: remember the authoritative payload — ``_try_fresh_final`` uses it
        # on split turns to detect non-prefix rewrites that must keep the legacy
        # edit path so the gateway can resend with a reply reference.
        self._authoritative_final_text = final_raw
    ...
    # ── fork overrides on the transport mixin (reply-ping only on the final answer,
    # ── QuixThe2nd/hermes-ide #48) ─────────────────────────────────────────
    # Discord notifies the referenced user on every MessageReference, and cannot
    # attach one via message.edit at all: interim previews / segment preambles must
    # stay standalone (no notify metadata, no reply anchor) and the completed answer
    # is delivered as ONE fresh reply.  These override the mixin defaults only where
    # turn-finality changes the outcome.

    # Set by _send_or_edit below for the duration of a send; read by _first_send.
    _first_send_turn_final: bool = True

    async def _send_or_edit(
        self, text: str, *, finalize: bool = False, is_turn_final: bool = True) -> bool:
        # Remember turn-finality so the first-send's notify metadata (the reply-ping
        # signal) is only set on the turn-final answer, never a segment preamble.
        self._first_send_turn_final = bool(is_turn_final)
        return await super()._send_or_edit(text, finalize=finalize, is_turn_final=is_turn_final)

    async def _first_send(self, text: str, *, finalize: bool) -> bool:
        """First send, threaded to the user's message (correct topic/thread).

        Fork #48 delta: a finalize that closes an interim SEGMENT (finalize=True,
        is_turn_final=False) is not notify-worthy — Discord would attach a reply
        reference (ping) to a preamble."""
        if getattr(self, "_egress_declined", False):
            ...
            return False
        notify_final = finalize and self._first_send_turn_final
        result = await self.adapter.send(
            chat_id=self.chat_id, content=text, reply_to=self._initial_reply_to_id,
            metadata=self._metadata_for_send(final=notify_final, expect_edits=not finalize))
    ...
    async def _edit_existing(self, text: str, *, finalize: bool, is_turn_final: bool) -> bool:
        ...
        Fork #48 deltas on the mixin: an adapter that prefers fresh final only gets
        it on the TURN-final (interim segments keep plain edits), and a failed
        turn-final fresh-final returns False instead of falling back to edit-in-place
        — the gateway's notify send must still deliver one reply ping."""
        # REQUIRES_EDIT_FINALIZE adapters need the finalize=True edit even when
        # unchanged; prefers-fresh adapters need it too (the same-text preview must
        # still be replaced by the reply-referenced fresh final).  Everyone else
        # short-circuits.
        ...
        if finalize and (
            (prefers_fresh and is_turn_final)
            or (not has_prefers_hook and self._should_send_fresh_final())
        ):
            if await self._try_fresh_final(text, is_turn_final=is_turn_final):
                return True
            # Discord (and any adapter that prefers fresh final) cannot attach a
            # reply reference via edit.  A failed fresh-final must not fall back
            # to edit-in-place and mark the turn delivered — leave final delivery
            # to the gateway's notify send so the user still gets one reply ping.
            if is_turn_final and prefers_fresh:
                return False
    ...
    async def _try_fresh_final(self, text: str, *, is_turn_final: bool = True) -> bool:
        ...
        Fork #48 delta on the mixin: on a multi-message split the caller's ``text`` is
        only the tail — the sealed heads carry the rest — so instead of refusing, the
        fresh send carries the full ``_stream_ledger`` (cleaned + fence-closed) WITH the
        reply reference.  Non-prefix rewrites of the authoritative final keep the legacy
        edit path so ``delivered_final_matches`` reports a mismatch and the gateway
        resends with the reply reference (#78541 / Discord contract)."""
        send_text = text
        if self._turn_split_delivery is True:
            ...
            if auth is not None and not auth.startswith(self._stream_ledger):
                return False
            send_text = self._display_payload(self._stream_ledger)
            if not send_text:
                return False
        stale_ids = self._stale_preview_ids()
        try:
            result = await self.adapter.send(
                chat_id=self.chat_id, content=send_text,
