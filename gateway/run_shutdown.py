...
class GatewayShutdownMixin:
    ...
    async def _notify_active_sessions_of_shutdown(self) -> None:
        ...
        for platform, adapter in list(self.adapters.items()):
            ...
            async def _send_notification_channel(adapter=adapter, channel=channel, platform=platform, metadata=metadata):
                if await self._send_shutdown_notice(
                    ...
                    **({"metadata": metadata} if metadata else {}),
                    ...
                    notified.add(dedup_key)
            from gateway.warning_notifications import present_notification
            await present_notification(_send_notification_channel, platform=platform)
