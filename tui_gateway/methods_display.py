...
privilege is asked for through the masked ``display.install.sudo`` card (the shared server→client
request bridge), stdout streams as ``display.install.log`` and the run ends with
...
def _(rid, params: dict) -> dict:
    ...
    def _ask_password() -> str:
        # App-level card, no session: `_ask` embeds sid in params.session_id, which write_json
        # routes to that session's transport — a client-supplied session_id could route the masked
        # password card into another window's chat. "" skips that route and reaches the connection
        # that clicked Install through the transport bind_transport() carries below.
    ...
    # the worker must inherit the handler's profile context or a multiplexed gateway would install
    # and snapshot against the launch home.
    from agent.memory_provider import spawn_context_thread
    spawn_context_thread(_run, name=f"bot-desktop-install:{profile_key}", daemon=True).start()
