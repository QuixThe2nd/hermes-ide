...
# Fork (hermes-ide): NO sensitive-system-path write refusal. write_file/patch do
# not refuse /etc, /boot, /usr/lib/systemd, or docker.sock paths — this is a
# root-owned single-operator distribution and the refusal just forced a
# /tmp + cp detour (fork commit 8b974bb3aa). Kept: the Hermes config-file write
# guard below (with its operator opt-out) and every other deny rule.
