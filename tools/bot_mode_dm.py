...
def _spawn_delivery(
    ...
    *,
    ...
    Relay deliveries pass the envelope id as ``delivery_id`` instead. The ack
    is ``queued`` + ``delivery_id`` like the live-owner branch: hand-off to a
    background process, never a delivery receipt.
    """
    transferred = False
    try:
        ...
        transferred = True
        if parsed.get("notify_on_complete") is False and parsed.get("notify_unsupported"):
            ...
            detail = (
                f"Message queued for {label}: this acknowledges the hand-off to a "
            ...
            reply_delivery = "notification" if notify_on_complete else "none"
        return json.dumps(
            {
                "status": "queued",
