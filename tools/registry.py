...
class ToolEntry:
    ...
    # Hidden (advertise=False) entries stay DISPATCHABLE forever but are excluded from
    # get_definitions(), so the model never sees them. Used for renamed tools' legacy
    # aliases: a replayed or hand-written call with the old name keeps working without
    # paying the schema cost of a second advertised tool.
    advertise: bool = True
