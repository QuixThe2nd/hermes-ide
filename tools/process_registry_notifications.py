...
def _format_async_delegation(evt: dict) -> str:
    ...
    ``result_kind`` (uniform delegation lifecycle) widens the vocabulary: a claude-code
    run, a Cursor cloud run, or an assistant mission renders with its own header and
    lead sentence instead of subagent phrasing. Absent on legacy events, which degrade
    to the original subagent wording."""
    ...
    # Subagent events carry role/model/api_calls; cli/cloud/mission units
    # generally do not — render only what the event actually provided.
    role, model = evt.get("role"), evt.get("model") or "?"
    if not role:
        if model != "?":
            lines[-1] = f"Model: {model}"
        else:
            lines.pop()
    ...
    if status in _DONE and summary:
        ...
        if status == "interrupted":
            lines.append("The delegated work was interrupted before completing" + (f": {error}" if error else "."))
        elif status == "cancelled":
            lines.append("The delegated work was cancelled." + (f"\n{error}" if error else ""))
        else:  # error / timeout / failed / unknown
            lines.append(
                f"The delegated work did not complete successfully (status={status})." + (f"\n{error}" if error else ""))
        if summary:
