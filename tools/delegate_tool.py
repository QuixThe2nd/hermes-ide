...
# Fork delta (rename): the primary tool name is ``delegate_agent``;
# ``delegate_task`` stays dispatchable as a hidden legacy alias (see the dual
# registration below). The entry function keeps upstream's name so merges stay
# small-surface; this alias is what direct Python callers (run_agent) import.
delegate_agent = delegate_task
...
DELEGATE_TASK_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            # Fork delta (background semantics): upstream deprecated/unadvertised
            # the `background` parameter; the fork deliberately keeps it ADVERTISED
            # because omitted-vs-explicit decides the delivery mode here (cron /
            # one-shot sessions block inline — see resolve_background_arg).
            "background": {
                "type": "boolean",
