...
process on THIS host. Privilege comes from the same masked server→client sudo card the terminal tool
raises (here the ``display.install.sudo`` request): ``sudo -n true`` is probed first (NOPASSWD /
cached timestamp hosts never see a prompt); when
