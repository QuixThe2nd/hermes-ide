...
def read_remote_roster(root: Path | str) -> list[dict]:
    ...
    try:
        data = json.loads((relay_root(root) / ROSTER_FILE).read_text(encoding="utf-8-sig"))
...
def local_delivery_base_command(
    ...
    *,
    ...
    argv = [_hermes_cli(), "-p", profile]
    argv += BOT_CHAT_TURN_ARGS if chat_title == "Bot Chat" else (
        "chat", "--in", "~", "-c", chat_title, "--create-if-missing", "-Q")
    if toolsets:
