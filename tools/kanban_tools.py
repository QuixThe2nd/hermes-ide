...
def _reject_delegated_child_mutation(tool_name: str) -> None:
    """A delegate_agent child shares the parent's process, so inherited HERMES_KANBAN_*
    ...
    if _delegation_ctx("is_delegated_child_process_context", False):
        raise _Reject(
            f"{tool_name} refused: delegate_agent child agents are not Kanban run owners. "
