...
def register(clarify_id: str, session_key: str, question: str, choices: Optional[List[str]],
             multi_select: bool = False, *, owner_profile: Optional[str] = None,
             owner_session_id: Optional[str] = None,
             wait_kind: Optional[str] = None) -> _ClarifyEntry:
    ...
    entry = _ClarifyEntry(
        ...
        owner_profile=(str(owner_profile).strip() or None) if owner_profile is not None else None,
        owner_session_id=(str(owner_session_id).strip() or None) if owner_session_id is not None else None,
        wait_kind=(str(wait_kind).strip() or "clarify") if wait_kind is not None else "clarify")
    with _lock:
