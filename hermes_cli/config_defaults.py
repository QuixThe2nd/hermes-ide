...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # Optional one-time model-visible checkpoint warning before a finite
        # turn cap is exhausted. null = off; set a ratio strictly between 0
        # and 1 (for example, 0.75).
    ...
    "plugins": {
        ...
        # After a Python hook callback hits hook_callback_timeout, further fires of that callback
        # skip for this long (the thread pile-up guard); once the window expires the next fire
        # supersedes the abandoned worker (never joined). Must be positive; max 600.
        "hook_timeout_suppression_seconds": 60.0,
    ...
    # encrypted vault (`hermes vault add`, Desktop → Settings → Credential Vault) is on by default
    # (`vault.local.enabled`); set false to remove the five browser_vault_* tools from the schema.
