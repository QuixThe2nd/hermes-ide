...
def gate_manifest(
    ...
    if manifest.source == "bundled":
        ...
        # Fork convention: bundled plugins declaring ``default_enabled: true`` in plugin.yaml load
        # without a ``plugins.enabled`` entry (fork ships custom plugins on by default). ``plugins.disabled``
        # was checked above and still wins; "load" routes through requires_plugins ordering.
