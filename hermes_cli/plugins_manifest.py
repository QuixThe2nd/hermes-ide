...
_KNOWN_MANIFEST_FIELDS: Set[str] = {
    ...
    "requires_hermes", "default_enabled", "disabled_management", "python_runtime",
}
...
def _parse_disabled_management_field(raw: Any, key: str) -> str:
    """Normalize the bundled-only disabled-management module stem (fork)."""
    if raw is None:
        return ""
    if not isinstance(raw, str):
        logger.warning(
            "Plugin %s: disabled_management must be a module stem string; ignoring", key)
        return ""
    stem = raw.strip()
    if not stem:
        return ""
    if "/" in stem or "\\" in stem or stem.startswith("."):
        logger.warning(
            "Plugin %s: disabled_management %r must be a bare module stem; ignoring", key, raw)
        return ""
    return stem
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    return {
        ...
        # Fork fields: bundled-only default-on opt-out and disabled-management module stem.
        "default_enabled": data.get("default_enabled", False) is True,
        "disabled_management": _parse_disabled_management_field(data.get("disabled_management"), key),
    }
...
class PluginManifest:
    ...
    requires_hermes: str = ""
    portable: bool = False
    # Fork convention: bundled plugins (source == "bundled") declaring ``default_enabled: true`` load
    # WITHOUT a ``plugins.enabled`` entry — the fork ships custom plugins on by default. Ignored for
    # user-installed/project plugins (untrusted code stays behind the allowlist); ``plugins.disabled``
    # still wins.
    default_enabled: bool = False
    # Bundled-only: relative ``.py`` module stem (no path separators) imported when ``plugins.disabled``
    # skips normal ``register()``; must expose ``register_disabled(ctx)`` to register cleanup hooks and
    # management CLI only — never full plugin capability. Ignored for user/project/entry-point plugins.
