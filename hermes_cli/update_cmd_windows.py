...
def _clear_windows_venv_holders_or_exit(args, gateway_mode: bool, _windows_gateway_resume):
    ...
    if getattr(args, "defer_restart", False):
        # Fork --defer-restart: every rung below STOPS or kills a holder, justified by the
        # post-update restart/resume phase a deferred run never reaches. So with the flag set no
        # holder is ever reaped — any holder is a straight refusal (exit 2), the correct outcome
        # for a prepare-only pass.
        if holders:
            print(_format_venv_python_holders_message(holders))
            print("✗ --defer-restart never stops a running Hermes runtime, so it")
            print("  cannot prepare while one holds the venv. Prepare when the")
            print("  runtime is closed, or run a plain `hermes update` instead.")
            _resume_and_exit()
        return
