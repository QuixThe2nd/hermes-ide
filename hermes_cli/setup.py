...
_DEFAULT_PROVIDER_MODELS = {
    ...
    "opencode-zen": ["gpt-5.6-sol", "gpt-5.4", "gpt-5.3-codex", "claude-opus-5", "claude-sonnet-5", "gemini-3.7-flash", "glm-5.2", "kimi-k3", "minimax-m3"],
...
def _apply_default_agent_settings(config: dict):
    ...
    config.setdefault("agent", {})["max_turns"] = DEFAULT_MAX_TURNS
    ...
    print_info(f"  Max iterations: {DEFAULT_MAX_TURNS}")
...
def setup_agent_settings(config: dict):
    ...
    current_max = str(cfg_get(config, "agent", "max_turns", default=DEFAULT_MAX_TURNS))
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    if section_key == "model":
        ...
        max_turns = cfg_get(config, "agent", "max_turns", default=DEFAULT_MAX_TURNS)
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    try:
        ...
        # A toolset that shares any tool with a kept one must stay off the
        # disabled list: ``disabled_toolsets`` subtracts at *tool* granularity,
        # so disabling e.g. ``file_readonly`` (read_file + search_files) would
        # strip those tools right out of the kept ``file`` toolset. Skipping it
        # is safe — ``cli`` has an explicit list, so nothing re-enables it.
        kept_tools: set = set()
        for ts in keep:
