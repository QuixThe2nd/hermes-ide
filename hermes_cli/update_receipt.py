    ...
    *, pre_restart_pids: Optional[list[int]] = None
    ...
    for profile, home in homes:
        try:
            ...
            # record still claims a running state — "the restart phase stopped it and nothing came
            # back." Everything else (clean stop, startup failure, long-dead stale record) keeps the
            # no-row behavior so the rollout can't false-positive. ``_pre_restart`` is a bare PID
            # set, not (pid, start_time) pairs, so a recycled PID from gateway A landing in B's
            # stale record could still mislabel B as down — inherent to the snapshot's data model.
