...
class PluginLoaderMixin:
    ...
    def _load_disabled_management(self, manifest: PluginManifest, loaded) -> None:
        """Import a bundled plugin's disabled-management module only (fork)."""
        with self._discovery_lock, _plugin_home_scope(self.home_path):
            self._load_disabled_management_scoped(manifest, loaded)

    def _load_disabled_management_scoped(self, manifest: PluginManifest, loaded) -> None:
        """Call ``register_disabled(ctx)`` without loading the main plugin (fork)."""
        from hermes_cli.plugins import DisabledPluginContext, _PLUGINS_DEBUG
        ...
        try:
            ...
            if register_fn is None:
                logger.warning(
                    "Plugin '%s' disabled_management module has no register_disabled()", plugin_key)
                return
            register_fn(DisabledPluginContext(manifest, self))
            registrations = [
                r for r in self._registration_order[registration_start:]
                if r.plugin_key == plugin_key and r.active
            ]
            loaded.hooks_registered = [r.key for r in registrations if r.kind == "hook"]
            loaded.commands_registered = [r.key for r in registrations if r.kind == "command"]
            loaded.tools_registered = [r.key for r in registrations if r.kind == "tool"]
            logger.debug(
                "  disabled-management registered: %d hook(s), %d CLI command(s)",
                len(loaded.hooks_registered),
                sum(1 for r in registrations if r.kind == "cli_command"),
            )
        except Exception as exc:
            ...
            logger.warning(
                ...
                plugin_key, exc, exc_info=_PLUGINS_DEBUG)

    def _load_disabled_management_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import ``disabled_management`` without executing ``register()`` (fork)."""
