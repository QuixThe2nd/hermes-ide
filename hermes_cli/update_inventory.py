...
def _collect_ledger_runtimes(plan: UpdatePlan, seen: set[int]) -> None:
    ...
    A backend whose pid is the MainPID of an active ``hermes-serve*`` unit is classified
    ``systemd`` — the update's restart phase restarts exactly those units alongside the
    gateways, so the plan must not book it as a manual serve nothing restarts."""
