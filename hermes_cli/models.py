...
#
# NOTE(fork): upstream rotates the silent default through
# ``models_catalog_static.PREFERRED_SILENT_DEFAULT_MODEL`` (currently
# ``z-ai/glm-5.2``); the fork pins the reference gateway's primary
# (xai-oauth + grok-4.6) and re-asserts the pair here after that import.
...
# NOTE(future-me): fork-union restore. Upstream 998f614c7f removed the keyless opencode-free
# tier and deleted _OPENCODE_FREE_EXCLUDED_MODELS / is_opencode_zen_free_model with it, but this
# file's catalog paths still reference the constant (live-list exclusion, #111749) and fork
# tests pin the classifier. Remove when: fork catalog stops excluding these ids AND no test
# references is_opencode_zen_free_model.
_OPENCODE_FREE_EXCLUDED_MODELS = frozenset({"ox-alpha-free", "deepseek-v4-flash-free"})
