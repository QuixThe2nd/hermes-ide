...
from hermes_cli.update_cmd_fleet import (  # noqa: F401
    ...
    PreparedGeneration, _activate_pending_fleet_restart_strict,
    _finish_deferred_restart, _fleet_restart_pending_prepared, _fleet_restart_prepared_path,
    _parse_prepared_generation, _publish_no_update_repair_generation, _publish_prepared_generation,
    _ACTIVATION_VERIFY_BUDGET_SECONDS, _PREPARED_RECORD_MAX_BYTES, _atomic_write_text,
    _current_checkout_head, _expected_gateway_profiles_from_receipt, _inspect_live_fleet_strict,
    _live_fleet_already_serves_checkout, _match_runtimes_injectively, _normalize_prepared_sha,
...
class _UpdateOptions:
    ...
    # Fork --defer-restart: prepare only; the fleet-restart obligation stays pending for a later
...
def _handle_update_called_process_error(
    ...
    zip_fallback_ok = _should_zip_fallback_on_update_error(e)
    if zip_fallback_ok and not getattr(args, "defer_restart", False):
        print(f"⚠ {stage}: {e}")
        ...
        if zip_fallback_ok and getattr(args, "defer_restart", False):
            # The ZIP fallback restarts the fleet, so a deferred run keeps this failure a failure
...
def _finish_already_up_to_date(
    ...
    current_checkout_complete, repaired_runtime = _repair_current_checkout(
        assume_yes=assume_yes, gateway_mode=gateway_mode,
        pre_update_snapshot_id=pre_update_snapshot_id,
        had_desktop_app_before_update=had_desktop_app_before_update,
        active_lazy_features=active_lazy_features,
    ...
    _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
    if defer_restart:
        # A deferred run never restarts anything — including a restart a *previous* run still owes.
        # But a repair that rewrote the venv owes the fleet a reload of the CURRENT HEAD, so publish
        # it as a SHA-bound prepared generation instead of leaving a bare, unprovable obligation.
        # The same publication gate as the HEAD-advanced path: a run whose completion verification
        # failed rewrote the venv but could not prove the run complete, so it must not publish.
        if not current_checkout_complete:
            deferred_defects.append(
                "update completion verification failed — the repair is only partially complete")
        _finish_deferred_restart(
            prepared_update=False, repaired_runtime=repaired_runtime, defects=deferred_defects)
        return
...
_POST_SWAP_FORWARDED_FLAGS = (
    ...
    # Fork --defer-restart: the post-swap child owns the preparation tail, so the deferred
    # contract only survives the hand-off when the flag crosses as argv.
    ("defer_restart", "--defer-restart"),
)
...
def _finish_pulled_update(
    ...
    if defer_restart:
        ...
        # non-deferred run (or an idle auto-update activation) picks the obligation up.
        _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
        if gateway_mode:
            _write_gateway_update_exit_code(desktop_build_ok)
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # --defer-restart must not stop gateways either: the pause exists so the venv can be mutated
    ...
    if use_zip_update:
        if defer_restart:
            # The ZIP fallback (Windows, git unusable) is one monolithic swap+build+restart
            # transaction: it has no prepare-only half, restarts the dashboard/serve fleet itself,
            # and never writes the fleet-restart marker. Refuse rather than half-honor the flag —
