...
from hermes_cli.tools_config_mcp import (  # noqa: F401
    _configure_mcp_tools_interactive, _apply_toolset_change, _apply_mcp_change,
    _print_tools_list, _known_tool_platforms, tools_disable_enable_command)
...
CONFIGURABLE_TOOLSETS = [
    ...
    ("delegation",      "👥 Task Delegation",           "delegate_agent"),
...
def _get_platform_tools(config: dict, platform: str, *, include_default_mcp_servers: bool = True) -> Set[str]:
    ...
    # #38798: an unresolvable name must never be echoed back as an "enabled" toolset — the caller
    # would believe the platform has it while get_toolset() resolves zero tools. Enabled MCP server
    # names and the "no_mcp" sentinel are not toolsets, but they DO resolve to tools, so they never
    # enter this set and keep flowing through unchanged.
    enabled_mcp_servers = enabled_mcp_server_names(config)
    from toolsets import validate_toolset

    explicit_passthrough -= {
        n for n in toolset_names
        if n != "no_mcp" and n not in enabled_mcp_servers and not validate_toolset(n)
    }
    ...
    if explicitly_configured and toolset_names:
        _warn_all_invalid_platform_toolsets(platform, platform_toolsets[platform], enabled_mcp_servers)
...
def _warn_all_invalid_platform_toolsets(platform: str, explicit: list, enabled_mcp_servers: Set[str] = frozenset()) -> None:
    """Warn once (per platform per process) when an explicit platform list references names that resolve to
    zero tools (#38798), at session tool resolution rather than only in update/doctor. A partially-valid list
    still resolves its valid toolsets, so only the bad names are flagged; an all-invalid list (``hermes`` for
    ``hermes-cli``) additionally warns that the platform has no native tools. Enabled MCP server names resolve
    to tools and are never flagged."""
    from toolsets import validate_toolset

    named = [str(t) for t in explicit if isinstance(t, str) and t]
    if not named or platform in _warned_invalid_platform_toolsets:
        return
    unknown = [n for n in named if n != "no_mcp" and n not in enabled_mcp_servers and not validate_toolset(n)]
    any_valid = any(validate_toolset(n) for n in named)
    if not (unknown or not any_valid):
        return
