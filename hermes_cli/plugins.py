...
VALID_HOOKS: Set[str] = {
    ...
    "pre_verify",
    # pre_turn_end (fork): end-of-turn gate fired once per turn right after the pre_verify gate
    # on EVERY turn; same directive contract ({"action": "continue", "message"} / Claude-Code Stop
    # {"decision": "block", "reason"}). Bounded by agent.max_pre_turn_end_nudges (default 1, cap 2).
    "pre_turn_end", "pre_api_request", "post_api_request", "api_request_error",
...
class PluginContext:
    ...
    def register_tool(
        ...
        description: str = "", emoji: str = "", override: bool = False, advertise: bool = True,
    ) -> Optional[PluginRegistration]:
        ...
        ``advertise=False`` (fork) registers a hidden alias: the name stays DISPATCHABLE forever
        (replayed transcripts, cached tool schemas, third-party scripts) but never appears in
        ``get_definitions()``, so it costs no schema tokens. Used by renamed tools to keep the old
        spelling working (uniform delegation lifecycle).
        """
        ...
        registry.register(
            ...
            override=override, scope=scope, advertise=advertise,
        )
...
class PluginManager(PluginLoaderMixin, PluginDispatchMixin, PluginLedgerMixin):
    ...
    def _gate_manifest(
        ...
        if verdict.action == "load_now":
            ...
            loaded = LoadedPlugin(manifest=manifest, enabled=verdict.enabled, error=verdict.error)
            self._plugins[manifest_key(manifest)] = loaded
            # Fork: a bundled plugin carrying ``disabled_management`` still imports its
            # capability-limited management module when disabled via config.
            if (verdict.error == "disabled via config" and manifest.source == "bundled"
                    and manifest.disabled_management):
