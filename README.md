...
The biggest piece of that machinery is Discord. This fork is **Discord First**: Discord isn't just another gateway adapter here, it is the primary operator surface, where the work gets driven from. Telegram, Slack, WhatsApp, Signal, and the CLI still work. Discord is where the house is. An optional per-channel response gate (OpenRouter Decisions, provider `jev`) decides whether unprompted messages deserve an answer, and its judge sees the bot's own recent replies too, so follow-ups to an ongoing conversation are recognized instead of denied. A sibling reaction gate, scoped and switched on separately, asks the same judge which single whitelisted emoji fits each eligible message and adds it only when the combined probability of "no reaction" and "no fitting emoji" stays strictly below one half — reacting without ever speaking.
...
Hermes Starts is what lets the agent speak first. It creates and pins its own Discord inbox, then uses each opening message as the anchor for a new thread. Any new bot message in that channel is auto-threaded the same way — whatever path delivered it, not only `start_conversation` openings. The plugin also lets the agent ask for changes to how it runs — its instructions, access, tools, limits, or working habits — when it has actually hit friction or spotted a concrete opportunity. Inbox Sparks adds a once-per-4-hour window where the agent must weigh starting a conversation before a turn ends. The inbox is allowed to be quiet; it is not allowed to exist only as decoration.
...
`llm_usage_proxy` is the other half of that picture: it records provider-reported token usage from real HTTP traffic on the wire (not estimates). It is on by default per profile (loopback-only); re-enable with `hermes llm_usage_proxy enable`, check the loopback proxy and SQLite path with `hermes llm_usage_proxy status`, and pull a rolling summary from `GET /usage/summary` on that proxy (there is no separate `summary` CLI verb yet). Opt out with `hermes llm_usage_proxy disable`, `llm_usage_proxy.enabled: false`, or a `plugins.disabled` entry. It only sees traffic that goes through Hermes's httpx client seams; anything that bypasses them (Cursor Cloud, an external CLI, a custom SDK client) is not captured, and providers that omit usage fields are stored as nulls.

**Key-manager mode** (`hermes llm_usage_proxy manage-keys on`) makes the proxy the only holder of provider API keys: they live in one root-only `<HERMES_HOME>/usage-proxy/keys.json` and clients get a local *caller token* instead, which the proxy swaps for the route's real credential on every request (rotating across a route's keys, retrying once on `401`/`429`). Every ledger row then names its caller in the `caller` column, so `/usage/summary` reports who spent a window as well as how much. Set keys and mint tokens with `hermes llm_usage_proxy keys set <route> --key <value>` (repeatable) and `hermes llm_usage_proxy callers create <name>` (printed once); `keys list` shows fingerprints only. Any OpenAI-compatible client then works with `OPENAI_BASE_URL=http://127.0.0.1:8790/p/zai` + `OPENAI_API_KEY=<caller token>`, Anthropic-protocol clients with `ANTHROPIC_BASE_URL=http://127.0.0.1:8790/p/zai-anthropic` + `ANTHROPIC_AUTH_TOKEN=<caller token>`, and Hindsight with `HINDSIGHT_API_LLM_BASE_URL=http://127.0.0.1:8790/p/zai` + `HINDSIGHT_LLM_API_KEY=<caller token>` — `/p/<route>/models` and `/p/<route>/v1/models` forward too, so model pickers keep working. With no caller tokens configured the proxy still serves labeled traffic without a token (Hermes's own routed traffic is unaffected); once one exists, unknown or missing tokens get a `401`. Caller attribution is enforced whenever the proxy manages keys: a request presenting neither a token, an `X-Usage-Caller` label, nor a recognized harness User-Agent (how Claude Code's probes name themselves) is refused with `401` and never reaches the upstream provider. OAuth providers (`openai-codex`, `xai-oauth`) are **not** manageable this way — they have no static key — so their routes stay passthrough. See `plugins/llm_usage_proxy/README.md` for the full detail.

`fallback_quota_reorder` uses the same score to rotate the primary and fallback list, moving the top scorer into the primary slot; an optional `fallback_quota_reorder.pinned_primary` mapping can keep a chosen route primary regardless of quota scores. Wallets that reset sooner rank higher. The Codex rotation default is `gpt-6-astra` (config migration v42 advances older `gpt-5.6-sol` entries; Sol stays manually selectable). `fallback_watch` tails `agent.log` and alerts Discord when the primary model falls back; it is opt-in, off by default, and cooldown-deduped.
...
Stock Hermes is also inconsistent about when it pings you, so you end up jumping between threads checking which one needs input. Hermes IDE avoids pinging you on iterations or mid-run messages, but will reply to you or ping you on the final message. I found this simple change improved my productivity a lot, because I stopped burning mental bandwidth rotating between 20 chats for hours straight waiting for one to complete. The long-running heartbeat is a phase line by default: it names the current wait and how long that wait has lasted — `⏳ terminal 1m42s` while a tool is in flight, `⏳ grok-4.6 38s` while waiting on the next model response, `⏳ packing 12s` once the last tool has returned and the next call is being packed — with no iteration counter or provider diagnostics. Set `display.platforms.discord.long_running_notifications: true` for the old verbose diagnostic line, `generic` for the plain `⏳ still working` phrase, or `false` to turn heartbeats off.
...
`delegate_claude_agent` handles medium and large jobs through the Claude Code CLI, with headless `/goal` as the default lane. Give it a `/goal <condition>` and it keeps going until a model judge, verified through the claude-glm wrapper — bundled with Hermes and installed alongside the `hermes` command, primary glm-5.2 with a glm-5.3-flash fallback — says the condition has been met. If the goal is too long for Claude Code's 4000-character limit, Hermes puts it in a brief inside the workdir instead of aborting the run. Claude writes stream-json output to the `claude-runs` logs while it works, so you can tail the run instead of staring at a silent chat.
...
All four delegate tools (`delegate_agent`, `delegate_claude_agent`, `delegate_cursor_agent`, `delegate_assistant`) share one lifecycle: an omitted `background` runs the work in the background — the call returns a handle immediately and delivers exactly one completion later — unless the session cannot receive a late completion (cron jobs, one-shot runs, Kanban workers, stateless HTTP endpoints), where it blocks to completion inline instead. `background=false` always blocks inline for that turn, and `background=true` always detaches, refused up front with no work started where late delivery is impossible. Set `delegation.default_background: false` in config to make an omitted argument block again. `delegate_task`/`dispatch_assistant` still dispatch as hidden aliases of the renamed tools.
...
`/restart` waits for active sessions to finish naturally — with no cap, so a user-requested restart never interrupts in-flight work, however long the last session takes. New work is refused while it waits, and the restart proceeds only once every active session, cron run, and API run has finished. `agent.restart_after_turn_timeout` is legacy and ignored for this wait. Before the restart is queued, both `/restart` and the `restart` tool run a short read-only source check of the installed checkout — root-level Python files, Python under `agent/`, `cron/`, `gateway/`, `hermes_cli/`, `plugins/`, and `tools/`, plus `plugin.yaml` files under `plugins/` — and warn the requester in the same chat when uncommitted runtime source changes exist in that scope (or, honestly, when the check cannot finish); the warning is informational only — it never asks for confirmation, never blocks recovery and adds at most four seconds before queuing, is skipped for packaged installs, and `drift_watch` remains the separate periodic record. Plain user messages that arrive during the wait follow the busy-input policy: with `queue` or `steer` they are appended to a durable on-disk snapshot and replayed when the gateway comes back, starting the session's turn so the message is actually answered — once, even across a crash mid-replay (the next boot resumes whatever the crash claimed but had not yet delivered; a record leaves the queue only once its turn is on its way) — and records for a platform still offline at boot wait on disk until it reconnects, as does the whole queue if a retry write fails until storage recovers. With the default `interrupt` posture, or during an external quiesce with no restart requested, they get the honest drain notice instead, on the idle and busy paths alike. Busy sessions that were already promised "queued for the next turn" get the same durability. Slash commands outside the lifecycle/read-only allowlist still get the drain notice instead of being queued. On Discord the requester gets one ⏸️ embed in the same chat, and only their reaction asks the other live chats to park safely now; that reaction-time snapshot is the only set resumed after restart. The shutdown warning shows each accepted LLM park steer. Chats that received the warning get a matching ♻️ back-online notice after the gateway returns, including after a raw SIGTERM.
...
`auto_update` runs unattended Hermes updates through an independent Linux/systemd timer, controlled with `hermes auto_update status|enable|disable|reconcile`. The default schedule is four times a day (00:00, 06:00, 12:00, 18:00), with no randomized delay. Every tick prepares the available update even while Hermes is busy, and the fleet restarts onto a fully prepared update only once Hermes is idle, ignoring stale streaming or unanswered rows outside the idle window. Before each update, quick snapshots keep the newest three plus the smallest set of older snapshots still holding the only usable (integrity-verified) copy of a database the newer ones skipped or failed to capture — retention is recovery-aware, so a database too large for the per-file cap never loses its last good copy to pruning. Note the limitation: capped updates preserve older copies, they do not create a fresh copy of an oversized database; capture one with a manual snapshot or full backup.
...
Installs with a provisioned inbox channel (hermes_starts, or the home_server `#inbox`) get that behavior by default: a job created from a Discord session in the inbox's guild whose delivery would default to `origin` is rewritten at create time to the `inbox` token, giving every cron job its own job-named thread under the inbox instead of rooting in the chat it was created from. Explicit targets are never rewritten, installs without a provisioned inbox are unaffected, and `cron.inbox_delivery_enforce: false` opts out.

Config gains API retry backoff timing plus fallback chains for web search and web extract. `agent.tool_call_narration_guidance` is on by default and asks the model to briefly explain a tool call before it makes one. Parent agents run on a default 256-turn budget (`agent.max_turns`, the same default `run_agent.py`'s `main()` uses); set it higher or to `none`/`0` for no limit. The silent default — what a no-override install lands on when `model.default` is empty and no model was ever picked — is Grok-4.6 through xAI OAuth (`xai-oauth`), matching the reference gateway's primary; every implicit path shares one source of truth (`PREFERRED_SILENT_DEFAULT_PROVIDER`/`_MODEL` in `hermes_cli/models.py`, mirrored by the model catalog's `default` labels, with aggregators spelling it `x-ai/grok-4.6`). The GLM-5.3-Flash / GPT-5.6-Sol / Kimi-K3 fallback chain is unchanged. `security.allow_agent_config_writes` lets an operator opt out of the `write_file` and `patch` guard on the Hermes config file; it is off by default. `display.notify_on_complete` can send a native OS notification when a turn finishes, including over an SSH target, and is also off by default.
...
On Discord, automatic context compression reports as a single tool-style status instead of scattered notices: one `🗜️ context_compress · …` message appears when an attempt starts (`selecting compressor` until the provider route is actually chosen, then edited to name it while the summary runs) and is edited in place when the attempt resolves — `✅` with the real provider/model and before/after message counts only when compression genuinely commits (a deterministic local summary is labelled as such, never credited to a provider), or a short failure category (e.g. `summary provider timed out`) with a `/compress` retry hint when it does not. A cooldown warning after a failed attempt folds into the same message rather than posting again. The gateway's pre-agent session-hygiene pass gets the same card: it posts when the summary work starts, and when the turn-hold budget releases your message while the summary is still streaming, the card switches once to "still compressing in the background" and later resolves to `✅` at the fenced adoption boundary (or `⚠️` if nothing commits) — replacing the separate deferral/timeout notices it used to post. Other platforms see no card at all and keep their existing notices.

The usage-proxy ledger ships with a read-only web dashboard in `apps/usage-proxy-webui`: one shared, URL-persisted filter state (time range, harness, provider, model, chat type, searchable chat, route, outcome) narrows every card, chart, breakdown and event list at once, with drill-down from per-chat/per-harness rows and the model donut. Hermes's own routed transports name the chat each request serves (`X-Usage-Chat-*` headers, validated and stripped before anything reaches the provider): gateway chats attribute to their platform chat (threads stay distinct), cron runs to their durable job, CLI/TUI sessions to their session id — and rows without recorded identity stay an explicit `Unknown` rather than being guessed. The chart stacks by harness/model/chat/chat-type plus input/output and cached/uncached modes; the per-chat table sorts by requests and token splits. Model names carry their provider's brand — an inline SVG logo (OpenAI, Anthropic, xAI, OpenRouter from Simple Icons; clean initial badges for Z.ai and Kimi) beside every model name on the donut legend, model chart mode and events table, with the brand colour driving donut slices and per-model series; unknown providers keep the neutral palette, and caller chips carry matching identity colours — hermes blue, Claude orange for claude-code, teal for the hindsight family — with unknown callers keeping the rank palette. Python stdlib only, no external assets, binds loopback by default (`python3 server.py --port 9136`).
