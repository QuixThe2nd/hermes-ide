...
Agents can call the `restart` tool as well. When other sessions or background jobs are in flight, it pings the requester and waits indefinitely for the exact word `restart`; anything else cancels. If a different confirmed restart queues while a gate is still waiting, that gate resolves as `superseded` (never `cancelled`): the bounce came from elsewhere, and the resumed session sees a note naming the bounce. When this chat is the only active session, it skips the prompt and queues the restart outright. While it waits, the calling thread is temporarily titled `Restart Pending`. Once confirmed, it uses the same drain path as `/restart`. On Discord the confirmation is one embed that pings the requester, without a second progress bubble sitting beside it.
...
The usage-proxy ledger ships with a read-only web dashboard in `apps/usage-proxy-webui`: request/token summaries per model and per harness, recent events, and a 24-hour stacked token chart with harness/model breakdowns plus input/output and cached/uncached modes. Harness breakdowns give every distinct caller its own series — the gateway's caller shows as `Hermes IDE`, and per-profile gateway callers (`hermes:<profile>`) appear as `Hermes IDE · <profile>` subcategories, indented under a Hermes IDE subtotal in the per-harness table. Python stdlib only, binds loopback by default (`python3 server.py --port 9136`).
