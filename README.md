...
- **Tools**
  ...
  - Delegation — `delegate_claude_agent` gains opt-in `remote_control=true`: the task runs on your locally authenticated first-party Claude Code (bare `claude` on a real PTY, never the GLM/Kimi wrappers) and returns a `progress_url` so you can watch and steer the run live at claude.ai/code or the Claude mobile app
