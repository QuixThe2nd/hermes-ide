...
The usage-proxy ledger ships with a read-only web dashboard in `apps/usage-proxy-webui`: request/token summaries per model and per harness, recent events, and a 24-hour stacked token chart with harness/model breakdowns plus input/output and cached/uncached modes. Model names carry their provider's brand — an inline SVG logo (OpenAI, Anthropic, xAI, OpenRouter from Simple Icons; clean initial badges for Z.ai and Kimi) beside every model name on the donut legend, model chart mode and events table, with the brand colour driving donut slices and per-model series; unknown providers keep the neutral palette. Python stdlib only, no external assets, binds loopback by default (`python3 server.py --port 9136`).
