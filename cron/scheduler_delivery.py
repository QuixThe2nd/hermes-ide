...
def _open_continuable_cron_thread(
    job: dict, adapter, chat_id: str, loop, thread_name: Optional[str] = None
) -> Optional[str]:
    ...
    ``thread_name`` overrides the default title (the thread: token's job-derived name)."""
...
def _resolve_single_delivery_target(
    ...
    # thread:<parent> / thread:<platform>:<parent> — the auto-created delivery-thread
    # token (helpers live in cron.scheduler, reached late-bound), parsed before the
    # generic platform:chat_id split below so the token's first segment is never misread
    # as a platform name. The failure lane resolves it to the plain parent chat (never
    # auto-creates); see _resolve_thread_delivery_target.
    thread_token = _sched._parse_thread_deliver_token(deliver_value)
...
class _TargetDelivery:
    ...
    # thread: deliver token (fork): this target must auto-create its delivery thread.
    thread_auto: bool = False
    deliver_token: Optional[str] = None
...
def _prepare_target_delivery(
    ...
    thread_auto = bool(target.get("_thread_auto"))
    if (
        mirror_this_target
        and not in_channel_surface
        # A thread: token's target creates ONE persistent thread (persisted onto the
        # job below) — the token's opt-in outranks this per-run open-a-fresh-thread
        # surface, and both firing would mint two threads on the first delivery.
        and not thread_auto
        and runtime_adapter is not None
        and loop is not None
    ...
    # thread: deliver token — this target came from the token and has no thread_id yet,
    # so open the job's dedicated delivery thread NOW (first delivery only: persistence
    # below rewrites the token to the concrete platform:parent:thread target, so no later
    # run or worker replay can create a duplicate). Needs the live adapter and a running
    # loop, the same gate as the send itself; when creation is impossible (no live
    # adapter, platform without threads, permissions) deliver flat on the parent chat for
    # this run — a missing thread must never fail the run.
    if thread_auto and not thread_id:
        new_thread_id = None
        ...
        if live_adapter_ready:
            new_thread_id = _open_continuable_cron_thread(
                ...
                thread_name=_sched._thread_autocreate_name(job, platform_name),
            )
            if not new_thread_id:
                no_create_reason = "thread creation returned nothing (unsupported platform or permissions)"
        if new_thread_id:
            thread_id = new_thread_id
            # Ride the same opened_thread_id path as the continuable open above so an
            # enabled mirror seeds the NEW thread's session (a reply in the fresh thread
            # sees the brief) instead of the parent chat's.
            opened_thread_id = new_thread_id
            if _sched._persist_thread_delivery_token(
                job, target.get("_deliver_token"), platform_name, chat_id, new_thread_id,
            ):
                logger.info(
                    "Job '%s': created delivery thread %s:%s:%s for the thread: deliver "
                    "token and persisted it on the job",
                    job["id"], platform_name, chat_id, new_thread_id)
        else:
            logger.warning(
                "Job '%s': thread: deliver token could not create a thread on %s:%s (%s) "
                "— delivering to the parent chat this run",
