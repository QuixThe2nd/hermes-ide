...
class MemoryManager:
    ...
    # providers get just the user's instruction (None for a bare invocation). Gateway
    # sessions may prepend Discord metadata and shared-session sender prefixes, so
    # those are stripped first via ``strip_gateway_scaffolding``.
    @staticmethod
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        return extract_user_instruction_from_skill_message(strip_gateway_scaffolding(text))
