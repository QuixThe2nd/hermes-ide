...
# pre_turn_end inject a synthetic user nudge to keep the agent going one more
# turn. These nudges must be stripped from returned/live history to avoid
# role-alternation breaks and poisoning the resumed transcript. The assistant
# response is real content and is not flagged. (#65919 §7)
...
def _resolve_budget_fallback(
    ...
    _pending_verification_response, _pending_verification_response_previewed, turn_api_time,
    logger,
) -> Tuple[Any, Any, bool, int, float]:
    """Iteration-budget exhaustion. Returns ``(final_response, _turn_exit_reason,
    preserved_verification_fallback, api_call_count, turn_api_time)`` — the summary
    call's cost is folded into the returned call count and timing so the caller's
    ``result`` metadata reflects it."""
    budget_exhausted = (
    ...
    return final_response, _turn_exit_reason, preserved_verification_fallback, api_call_count, turn_api_time
...
def finalize_turn(
    ...
    (
        final_response, _turn_exit_reason, preserved_verification_fallback,
        api_call_count, turn_api_time,
    ...
    # Plugin hook: on_session_end — fired at the very end of every
    # run_conversation call (cleanup, flushing buffers, etc.).
    if not getattr(agent, "_persist_disabled", False):
        _invoke_hook_safely(
