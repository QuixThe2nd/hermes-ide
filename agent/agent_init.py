...
_CONTROL_STATE: Dict[str, Any] = {
    ...
    "_turn_gateway_restart_queued": False,
    ...
    # One-shot notification lane for the steer mechanism above: listeners fire exactly
    # when a pending steer is ACTUALLY injected into the model's context (the two mid-run
    # drain points), never on the put-back paths or the end-of-turn leftover drain.
    # Surfaces (e.g. the gateway's "steer delivered" ack) register/unregister via
    # AIAgent.register_steer_delivery_listener(); zero-cost when empty.
    "_steer_delivery_listeners": list,
    "_steer_delivery_listeners_lock": threading.Lock,
...
def _apply_agent_section(agent, _agent_cfg):
    ...
    # tool_call_narration_guidance is separate from parallel_tool_call_guidance so a user
    # can silence narration without losing parallel-call batching.
    for _key in (
        "stall_guards", "task_completion_guidance", "parallel_tool_call_guidance",
        "tool_call_narration_guidance", "environment_probe", "bot_mode_protocol",
    ):
...
def init_agent(
    ...
    args: list[str] | None = None, model: str = "",
    ...
    Non-obvious parameters:
      max_iterations: default turn budget (DEFAULT_MAX_TURNS); the budget is shared with
        subagents; unlimited spellings map to sys.maxsize.
