...
def build_activity_snapshot(
    *,
    ...
    # How long the CURRENT wait has lasted. Unlike ``seconds_since_activity`` (refreshed by periodic
    # liveness heartbeats), the phase clock only restarts when the activity description changes, so a
    # wait longer than the heartbeat cadence still counts up. Unknown start (or a start in the future —
    # clock skew) stays None rather than going negative.
    return {
        ...
        "phase_seconds": (
            round(clock - phase_start, 1) if phase_start is not None and clock >= phase_start else None
        ),
