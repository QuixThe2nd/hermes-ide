...
def _read_text_with_timeout(path: Path, timeout: Optional[float] = None) -> Optional[str]:
    ...
    def _reader() -> None:
        try:
            result.put((True, path.read_text(encoding="utf-8-sig")))
...
HERMES_AGENT_HELP_GUIDANCE = (
    ...
    # skill will not ALSO fetch the docs to diff them, so the clause was dead
    ...
    "For Hermes IDE specific features, check "
...
KANBAN_GUIDANCE = (
    ...
    "specialist profile.\n"
    "- Do not call `delegate_agent` as a board substitute. `delegate_agent` is "
...
def _build_snapshot_entry(skill_file: Path, skills_dir: Path, frontmatter: dict, description: str) -> dict:
    ...
    if org_id:
        ...
        try:  # author from the pull-time provenance sidecar; best-effort
            prov = json.loads((skills_dir / ORG_MIRROR_DIR_NAME / org_id / ORG_PROVENANCE_FILE).read_text(encoding="utf-8-sig"))
