...
def _commit_tool_result(
    ...
    *,
    ...
    # Trusted terminal control: a restart result that committed the drain arms
    # the per-turn flag before any further transformation of the result.
    # Cancelled/failed restarts carry no control field and the flag stays
    # unarmed (the normal loop continues). The planner keeps restart on the
    # sequential barrier path, so the concurrent commit reaching here is
    # defense-in-depth for any future dispatch surface that lands a restart.
...
class _ConcurrentBatch:
    ...
    def await_completion(self, futures, future_to_index, deadline: float | None) -> bool:
        ...
        while True:
            ...
            if timed_out:
                # Delegation/restart-class tools are exempt from the batch deadline
                # (``timeouts.tools.unbounded_tools``; see _resolve_unbounded_tools):
                # long quiet stretches are normal for them. Drop the deadline when
                # only exempt tools remain; in a mixed batch, report the bounded
                # stragglers timed out and keep waiting on the exempt ones.
                _exempt_names = _resolve_unbounded_tools()
                exempt_futures = {
                    f for f in not_done
                    if f in future_to_index
                    and self.parsed_calls[future_to_index[f]].name in _exempt_names
                }
                ...
                if not bounded_not_done:
                    logger.info(
                        ...
                        self.timeout_s,
                        ", ".join(self._running_names(not_done, future_to_index)),
                    )
                    deadline = None
                    continue
                self.timed_out_indices = {future_to_index[f] for f in bounded_not_done if f in future_to_index}
                logger.warning(
                    "concurrent tool batch timed out after %.1fs; %d tool(s) still running: %s",
                    self.timeout_s,
                    len(self.timed_out_indices),
                    ", ".join(self._running_names(bounded_not_done, future_to_index)[:5]),
                )
                ...
                continue
            for f in (bounded_not_done if timed_out else not_done):
                f.cancel()
            ...
            if timed_out:
                if exempt_futures:
                    # Mixed batch: bounded stragglers are reported timed out and
                    # left detached (a per-thread interrupt here would also kill
                    # the exempt runs). Keep waiting on the exempt futures with
                    # no deadline.
                    logger.info(
                        "keeping %d exempt tool(s) running past batch deadline: %s",
                        len(exempt_futures),
                        ", ".join(self._running_names(exempt_futures, future_to_index)),
                    )
...
def _delegate_spinner_label(function_args: dict) -> str:
    ...
    # Mode-aware tail: a foreground run blocks here (the spinner is the only
    # "still working" signal the user gets); a background dispatch returns a
    # handle and the work continues outside the turn.
    _bg_flag = function_args.get("background")
    _is_bg = _bg_flag is True or (
        isinstance(_bg_flag, str) and _bg_flag.strip().lower() in ("1", "true", "yes", "on"))
    _tail = "/agents to monitor" if _is_bg else "waiting on results"
    if action in ("list", "steer", "stop"):
        return f"🔀 subagent {action}"
    if tasks and isinstance(tasks, list):
        return f"🔀 delegating {len(tasks)} tasks · ({_tail})"
    goal_preview = (function_args.get("goal") or "")[:30]
    return f"🔀 {goal_preview} · ({_tail})" if goal_preview else f"🔀 delegating · ({_tail})"
...
def _resolve_sequential_dispatch(agent, ref: _ToolCallRef, messages: list) -> _SequentialDispatch:
    ...
    if function_name not in ("delegate_agent", "delegate_task") and function_name in INLINE_TOOL_EXECUTORS:
    ...
    if function_name in ("delegate_agent", "delegate_task"):
        ...
        return _SequentialDispatch(agent._dispatch_delegate_agent, spinner=spinner, is_delegate=True)
...
def execute_tool_calls_segmented(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, segments=None) -> None:
    ...
    A terminal gateway-restart result drains the remaining segments the same way."""
