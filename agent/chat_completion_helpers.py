...
def should_use_direct_api_call(agent) -> bool:
    ...
    if getattr(agent, "platform", None) == "cron":
        return True
    # Delegated child (delegate_agent sync or background) — via the execution ContextVar set by _run_single_child,
...
def _iteration_summary_chat_kwargs(agent, api_messages: list) -> dict:
    ...
    if not is_lmstudio and agent._supports_reasoning_extra_body():
        ...
        if agent.reasoning_config is not None:
            extra_body["reasoning"] = _clamp_summary_reasoning(agent.model, agent.reasoning_config)
        else:
