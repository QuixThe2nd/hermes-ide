...
def _sum_delegate_task(name, args, content, content_len, line_count):
    ...
    return f"[{name}] '{goal}' ({content_len:,} chars result)"
...
_TOOL_RESULT_SUMMARIZERS = {
    ...
    **dict.fromkeys(
    ...
    "delegate_agent": _sum_delegate_task,  # fork: legacy spelling still registered
...
class ContextCompressor(SummaryDispatchMixin, MicroCompactionMixin, ContextEngine):
    ...
    def _begin_compression_telemetry(
        ...
        telemetry: Dict[str, Any] = {
            ...
            # True only when call_llm actually recorded the route it selected (route_info
            # populated); the aux_provider/aux_model fallbacks are config guesses, not proof
            # of the summarizer's identity — presentation must gate on this flag.
    ...
    def _record_aux_compression_call(
        ...
        aux_provider: str | None = None, aux_model: str | None = None, aux_route_known: bool = False,
...
Summary generation was unavailable, so this is a best-effort deterministic fallback for {len(turns_to_summarize)} compacted message(s).{reason_text}"""
    ...
    def _call_summary_llm(self, prompt: str, prompt_started_at: float) -> str:
        ...
        # The observing dict publishes that route to the presentation layer the moment it is
        # recorded — while the call is still in flight — and again if call_llm falls back to a
        # secondary route.
        ...
        try:
            ...
            # The live route was for THIS call only — never let a stale observed route leak
            # into the next attempt's presentation.
