...
updates:
  ...
  backup_keep: 3                 # Keep this many full pre-update backup zips
...
The **Delegation** entry is special: it routes the model used by `delegate_agent` subagents and persists to the top-level `delegation.*` section (`delegation.provider` / `delegation.model`) rather than `auxiliary.*`, because subagents are full child agents, not side-LLM calls. Its `auto` means "inherit the parent agent's provider, model, and credentials."
...
A single `delegate_agent` batch counts each task toward `max_subagents` (a batch of 3 spends 3), so the cap tracks real subagents spawned rather than `delegate_agent` invocations.
...
- `thread_free_response_channels` — comma-separated list of channel IDs that stay mention-free (they must also be in `free_response_channels`) yet still auto-thread each message instead of replying inline.
