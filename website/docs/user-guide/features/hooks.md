...
| `model` | `str` | The model identifier |
| `platform` | `str` | Where the session is running |
| `title` | `str` | Chat/session title (`display_name`, else `title`, else `"Hermes"`) — resolved by Hermes so hooks need no `state.db` lookup |
