...
3. The gateway watcher claims the handoff and asks the destination adapter for a fresh thread:
   ...
   - **WhatsApp / Signal / SMS** — no native threads, delivers to the notification channel directly.
...
**Limitation worth knowing:** for non-thread-capable platforms with multi-user group notification channels, the synthetic turn keys as a DM-style session. This works for self-DM notification channels (the typical setup) but isn't ideal for genuinely shared group chats. Threading covers Telegram / Discord / Slack / Matrix — by far the common case — so most setups never hit this.
